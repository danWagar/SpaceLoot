import { Screen, GameScreen, UpgradeScreen } from "../javascript/screen.js";
...
export default class GameState {
    ...
    addScore() {
        this.currentScreen.addTextBox("score: " + this.game.score, this._context.canvas.width - 150, 15, "yellow", "18px", "Georgia");
    }

    addGold() {
        this.currentScreen.addTextBox("GOLD: " + this.game.user.gold, this._context.canvas.width - 150, 15, "yellow", "18px", "Georgia");
    }
    ...
    pause() {
        ...
        if( this.currentScreen === this.pauseScreen ){
            ...
            this.pauseScreen.addTextBox(txt, this._context.canvas.width/2 - txtWidth/2, this._context.canvas.height/2, "red", "45px", "Georgia");
    ...
    win(score) {
        ...
        this.winScreen.addTextBox(txt, this._context.canvas.width/2 - 98, this._context.canvas.height/2, "red", "30px", "Georgia");
        this.addScore();
        this.winScreen.draw();
    }
    ...
    upgrade() {
        ...
        for(let i = 0; i < this.upgradeScreen.upgradeImages.length; i++) {
            ...
            console.dir(dim);
            console.log(`${this.upgradeScreen.upgradeImages[i].name} currentLvl is ` + this.game.player.upgradeables[i].currentLvl);
            while(j < this.game.player.upgradeables[i].currentLevel) {
                padding = j * 4;
                offset = 4 * this.game.player.upgradeables[i].maxLevel / 2;
                this.upgradeScreen.addRect("yellow", dim.x + dim.width/4 + rectWidth * j + padding - offset, dim.y + dim.height - rectHeight, rectWidth, rectHeight);
                console.dir(this.upgradeScreen);
                j++;
            }
            console.dir(this.game.player.upgradeables[i]);
            console.log(`${this.upgradeScreen.upgradeImages[i].name} maxLevel is ` + this.game.player.upgradeables[i].maxLevel);
            while( j < this.game.player.upgradeables[i].maxLevel ) {
                padding = j * 4;
                offset = 4 * this.game.player.upgradeables[i].maxLevel / 2;
                this.upgradeScreen.addRect("gray", dim.x + dim.width/4 + rectWidth * j + padding - offset, dim.y + dim.height - rectHeight, rectWidth, rectHeight);
                j++;
            }
        }
    ...
    gameOver() {
        ...
        this.gameOverScreen.addTextBox("GAME OVER", this._context.canvas.width/2 - 175, this._context.canvas.height/2 - 10, "red",  "60px", "Georgia" );
        this.addScore();
        this.game.reset(this._context);
