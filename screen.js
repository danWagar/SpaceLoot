export class Screen {
    ...
    checkButtonClick(event) {
        ...
        console.dir(this);
        for( let i=0; i<this.buttonArray.length; i++ ) {
            console.log("Button " + this.buttonArray[i].name + " coords are " + this.buttonArray[i].x + "," + this.buttonArray[i].y);
...
class Button {
    ...
    within(coords) {
        ...
        if( coords.x <= this.x + this.width && coords.x >= this.x &&
            coords.y <= this.y + this.height && coords.y >= this.y) {
                return true;
        }
        else{
            return false;
...
export class UpgradeScreen extends Screen {
    ...
    drawUpgradeImages(numCols, startHeight, vertPadding, border, color) {
        ...
        for(let i = 0; i < this.upgradeImages.length; i++) {
            ...
            x += (img.img.width + padding) * currCol;
            img.x = x;
            this.addImage(img.img, x, y, border, color, img.name )
            //add plus and minus buttons
            this.addButton(this._plusButton, x + 5, y + img.img.height - this._plusButton.height,
                           `plus${img.name}`);
            this.addButton(this._minusButton, x + img.img.width - 5 - this._minusButton.width, 
                           y + img.img.height - this._minusButton.height, `minus${img.name}`);
            if ( (i + 1) % numCols === 0 ) {
                currRow ++;
                currCol = 0;
